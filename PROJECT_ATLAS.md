...
| **Setting** | A key-value pair persisted in SQLite (hotkeys, auto-backup prefs, reminder prefs). |
...
│       │   ├── settings.rs   ← Hotkeys, autostart, auto-backup, reminder prefs
...
| **Settings** | `get_hotkey_settings`, `update_hotkey_settings`, `get_autostart_state`, `set_autostart`, `toggle_autostart`, `store_auto_backup_password`, `has_auto_backup_password`, `delete_auto_backup_password`, `get_auto_backup_settings`, `update_auto_backup_settings` |
...
| `src-tauri/src/config.rs` | Backend | Application constants (window sizes) | Compile time |
...
- **Database settings** (`settings` table): hotkeys, auto-backup prefs, reminder prefs, theme — validated by services at read time.
