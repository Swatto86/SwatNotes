  ...
  describe('processSmartPaste', () => {
    describe('title detection', () => {
      ...
      it('should detect short lines followed by blank lines as titles', () => {
        ...
        // Find the title op
        const titleOp = result.ops.find(op => 
          typeof op.insert === 'string' && op.insert === 'Project Overview'
        );
    ...
    describe('list detection', () => {
      it('should convert bullet lists with dash', () => {
        ...
        const listOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any).list === 'bullet'
        );
      ...
      it('should convert ordered lists', () => {
        ...
        const listOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any).list === 'ordered'
        );
        expect(listOps.length).toBe(3);
      });
      ...
      it('should handle nested lists with indentation', () => {
        ...
        const indentedOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any).indent === 1
        );
    ...
    describe('URL detection', () => {
      it('should detect and linkify URLs', () => {
        ...
        const linkOp = result.ops.find(op => 
          op.attributes && (op.attributes as any).link === 'https://example.com'
        );
      ...
      it('should handle www URLs and add https', () => {
        ...
        const linkOp = result.ops.find(op => 
          op.attributes && (op.attributes as any).link === 'https://www.example.com'
        );
        expect(linkOp).toBeDefined();
      });
      ...
      it('should detect email addresses', () => {
        ...
        const emailOp = result.ops.find(op => 
          op.attributes && (op.attributes as any).link === 'mailto:hello@example.com'
        );
    ...
    describe('code block detection', () => {
      it('should detect fenced code blocks', () => {
        ...
        const codeOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any)['code-block']
        );
...
For more details, visit https://project.example.com or contact team@example.com`;
        ...
        // Should have headers
        const headerOps = result.ops.filter(op => 
          op.attributes && ((op.attributes as any).header === 1 || (op.attributes as any).header === 2)
        );
        expect(headerOps.length).toBeGreaterThan(0);
        
        // Should have bullet list
        const bulletOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any).list === 'bullet'
        );
        expect(bulletOps.length).toBe(3);
        
        // Should have ordered list
        const orderedOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any).list === 'ordered'
        );
        expect(orderedOps.length).toBe(3);
        
        // Should have links
        const linkOps = result.ops.filter(op => 
          op.attributes && (op.attributes as any).link
        );
    ...
    describe('edge cases', () => {
      ...
      it('should preserve blank lines', () => {
        ...
        const blankOps = result.ops.filter(op => 
          op.insert === '\n' && !op.attributes
        );
