...
/**
 * Load behavior settings from backend
 */
async function loadBehaviorSettings(): Promise<BehaviorSettings> {
  try {
...
/**
 * Save behavior settings to backend
 */
async function saveBehaviorSettings(settings: BehaviorSettings): Promise<void> {
  await invoke('update_behavior_settings', { settings });
  logger.debug('Behavior settings saved', LOG_CONTEXT, settings);
}

/**
 * Load autostart state from backend (actual registry state)
 */
async function loadAutostartState(): Promise<boolean> {
  try {
    return await invoke<boolean>('get_autostart_state');
  } catch (error) {
    logger.error('Failed to load autostart state', LOG_CONTEXT, error);
    return false;
  }
}
...
function setupEventHandlers(): void {
  ...
  // Behavior settings (loaded from backend)
  loadBehaviorSettings().then((behaviorSettings) => {
    // Minimize to tray checkbox
    const minimizeToTrayCheckbox = document.getElementById(
      'minimize-to-tray-checkbox'
    ) as HTMLInputElement;
    if (minimizeToTrayCheckbox) {
      minimizeToTrayCheckbox.checked = behaviorSettings.minimize_to_tray;
    ...
    const closeToTrayCheckbox = document.getElementById(
      'close-to-tray-checkbox'
    ) as HTMLInputElement;
    if (closeToTrayCheckbox) {
      closeToTrayCheckbox.checked = behaviorSettings.close_to_tray;
    ...
    if (autoSaveDelayInput) {
      autoSaveDelayInput.value = behaviorSettings.auto_save_delay.toString();
      autoSaveDelayInput.addEventListener('change', async () => {
  ...
  // Start with Windows checkbox (loaded from actual registry state)
  loadAutostartState().then((isAutostartEnabled) => {
    const startWithWindowsCheckbox = document.getElementById(
      'start-with-windows-checkbox'
    ) as HTMLInputElement;
    if (startWithWindowsCheckbox) {
      startWithWindowsCheckbox.checked = isAutostartEnabled;
