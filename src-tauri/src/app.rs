...
pub fn setup(app: &mut App) -> Result<()> {
    ...
    // Setup window close handler to respect close_to_tray setting
    if let Some(window) = app.get_webview_window("main") {
        let window_clone = window.clone();
        let settings_service = state.settings_service.clone();
        window.on_window_event(move |event| {
            if let tauri::WindowEvent::CloseRequested { api, .. } = event {
                // Check if close_to_tray is enabled (default: true)
                let close_to_tray = {
                ...
                // If close_to_tray is false, allow the default close behavior
            }
        });
    }
