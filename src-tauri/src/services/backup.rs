...
pub struct BackupService {
    ...
    backups_dir: std::sync::Arc<std::sync::RwLock<PathBuf>>,
}
...
impl BackupService {
    pub fn new(repo: Repository, blob_store: BlobStore, app_data_dir: PathBuf) -> Self {
        ...
        Self {
            ...
            blob_store,
            app_data_dir,
            backups_dir: std::sync::Arc::new(std::sync::RwLock::new(backups_dir)),
        }
    }

    /// Set custom backup directory
    pub fn set_backup_dir(&self, path: PathBuf) -> Result<()> {
        let mut dir = self
            .backups_dir
            .write()
            .map_err(|e| AppError::Generic(format!("Failed to acquire lock: {}", e)))?;
        *dir = path;
        tracing::info!("Backup directory updated to: {:?}", dir);
        Ok(())
    }

    /// Get current backup directory (internal)
    ...
    /// Get current backup directory (public, for validation)
    pub fn get_backup_directory(&self) -> Result<PathBuf> {
        self.get_backup_dir()
    }
    ...
    pub async fn create_backup(&self, password: &str) -> Result<PathBuf> {
        ...
        if notes.is_empty() {
            return Err(AppError::Backup(
                "Cannot create backup: No notes found. Create at least one note before backing up."
                    .to_string(),
            ));
        }
        ...
        for hash in &blob_hashes {
            ...
            let blob_rel_path = format!("blobs/{}/{}/{}", &hash[0..2], &hash[2..4], hash);
    ...
    async fn apply_retention_policy(&self) -> Result<()> {
        ...
        let backups_to_delete: Vec<_> = sorted_backups.iter().skip(retention_count).collect();
        ...
        // Delete all database records atomically first
        // This ensures consistency - if any file deletion fails later,
        // orphaned files can be cleaned up manually (they won't appear in the UI)
        let ids_to_delete: Vec<String> = backups_to_delete.iter().map(|b| b.id.clone()).collect();
        self.repo.delete_backups_batch(&ids_to_delete).await?;

        // Now delete the files (non-atomic, but records are already gone)
        for backup in backups_to_delete {
            tracing::debug!("Deleting old backup file: {}", backup.path);
            if let Err(e) = fs::remove_file(&backup.path).await {
                tracing::warn!(
                    "Failed to delete backup file {} (will leave orphaned file): {}",
                    backup.path,
                    e
                );
    ...
    /// Clean up orphaned backup records (records without files)
    async fn cleanup_orphaned_records(&self) -> Result<()> {
        ...
        if !orphaned_ids.is_empty() {
            tracing::info!("Cleaning up {} orphaned backup records", orphaned_ids.len());
    ...
    /// List available backups (automatically cleans up orphaned records)
    ...
    pub async fn delete_backup(&self, backup_id: &str, backup_path: &str) -> Result<()> {
        tracing::info!("Deleting backup: {} ({})", backup_id, backup_path);
        ...
        if path.exists() {
            fs::remove_file(path)
                .await
                .map_err(|e| AppError::Backup(format!("Failed to delete backup file: {}", e)))?;
            tracing::debug!("Deleted backup file: {}", backup_path);
        }

        // Delete database record
        self.repo.delete_backup(backup_id).await?;
    ...
    pub async fn restore_backup(&self, backup_path: &Path, password: &str) -> Result<()> {
        ...
        let backups_dir = self.get_backup_dir()?;
        let temp_restore_dir = backups_dir.join(format!("restore_temp_{}", Utc::now().timestamp()));
        ...
        // Close the database connection pool to release file handles
        // This is required on Windows where files can't be renamed while open
        tracing::info!("Closing database connection pool for restore...");
        self.repo.close().await;
        tracing::info!("Database connection pool closed");

        // Small delay to ensure all handles are released (especially on Windows)
        ...
        let db_path = self.app_data_dir.join("db.sqlite");
        let db_backup_path = self
            .app_data_dir
            .join(format!("db.sqlite{}", backup_suffix));
...
mod tests {
    use super::*;
    use crate::database::{initialize_database, models, CreateNoteRequest, Repository};
    ...
    async fn test_retention_policy() {
        ...
        // Create a test note first (required for backup)
        service
            .repo
            .create_note(CreateNoteRequest {
        ...
        // Should have only 3 in DB (retention policy deletes both files and records)
